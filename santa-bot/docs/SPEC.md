...
- Backups are written under a temporary name and renamed, with mode 0600, and converted to a single self-contained file (journal_mode=DELETE). P1 S3 upload: a SigV4-signed path-style PUT (stdlib + aiohttp, checked against botocore); new optional variable `S3_REGION` (default ru-central1); a failed upload alerts the admins; old objects are removed by a bucket lifecycle rule the owner sets.
...
P1 features (built after P0 was green):
- Redraw (§5.5): [Перезапустить жеребьёвку] on the drawn panel with a confirmation, at most twice. Old pair messages still queued are dropped, so they never arrive after the new ones. `/stats` and the landing counter do not count redraws as draws. A reply to an anonymous message from before a redraw or a changed pair is refused ('Пары изменились…'), so it cannot reach someone who is no longer the counterpart. Exclusions stay editable only before the draw; a redraw that cannot satisfy them says so.
- [Подарок готов] on the draw result and in the game view; the drawn panel shows 'Подарки готовы: k из n'.
- Leaving or removal after the draw (§5.5): allowed with a confirmation that explains the splice; the organizer leaves through the same [Выйти из игры]. The leaver's Santa gets 'Ваш получатель выбыл…' with the new receiver's wishes; the organizer is asked to redraw when the new pair is excluded or fewer than 3 remain. Waiting people may also leave a drawn game.
- Reveal (§5.11): available once from the exchange date on (any time after the draw when there is no date), with a confirmation. The chain follows the cycle from the first participant who joined.
- Group mode (§5.10): the gc_ wizard sends no invite (the card is in the chat) and says so. The card is edited through a debouncer: at once if the last edit was 10 s ago or more, otherwise once when the 10 s are over, with the state at that moment (kept in memory; after a restart the next change edits the card). A cancelled game's card says so; a drawn game's card has no button. After a payment the card is refreshed in the background.
- tools/make_avatar.py draws the 500×500 PNG with the standard library (signed-distance shapes, zlib).
- Command menu (§9): MAX documents PATCH /me/commands (checked 2026-09-25), so the startup registers /start, /help, /cancel and /whoami (names sent without '/', which the docs do not specify); a failure is only logged. Admin commands stay out of the public menu.
Review fixes (2026-09-25):
- Redraw: the confirmation button is `rdy:{game}:{redraw_count}`; `run_draw(redraw_of=…)` refuses when the count moved on ('Жеребьёвку по этому подтверждению уже перезапустили'), so a double tap or an old confirmation cannot reshuffle the pairs twice. A redraw and a splice reset `gift_ready` of the affected Santas. After a splice the organizer gets a redraw request only when a redraw can help; with fewer than 3 people or no redraws left the text says so.
- Payments (§4, §5.6): a payment row keeps the amount it was priced at, so an older link paid after another upgrade of the same game overcharges or buys nothing. Confirmation computes the excess (all of it when the tier does not raise the limit; else paid sum minus the tier price) and alerts the admins to refund it (the §5.6 double-payment text is kept for the same tier); the payer is told the money will be returned, and the organizer hears nothing about a payment that changed nothing. A payment arriving after the draw or a cancellation is recorded as paid but not applied; the payer gets 'Оплата пришла, когда игра уже не набирала участников… вернём'. Waiting people in a drawn game see no 'я напишу' promise and may leave.
- Waiting texts: a paid game that is full says 'все N мест в игре заняты' instead of the free-tier sentence, using the game's own limit. At the top tier the organizer's waiting notice adds the 'напишите нам' e-mail (§4).
- Webhook: startup always posts the subscription (GET /subscriptions never shows the secret, and POST updates an existing one, dev.max.ru); the watchdog posts again while the subscription is not known to work or its update types differ, and removes this bot's subscriptions to other /max/webhook/ addresses (new `MaxApi.unsubscribe`, DELETE /subscriptions?url=). A failed subscription alerts the admins with MAX's reason at most hourly; a restored one says 'Подписка вебхука оформлена…'. A passing network error while the webhook works is only logged. dev.max.ru confirms that long polling does not work while a subscription exists; MODE=polling logs that as an error and does not delete the subscription (it may be production's).
- Outbox: a 401 no longer counts as an attempt (replaces the 'same backoff' note above): the row waits 5 minutes per try and is released at once when GET /me works after a restart, so fixing the token never loses queued messages.
- Backups: a day's backup file is never replaced, so the run made up after restoring an older copy cannot overwrite that morning's copy. InvIds are `max(last + 1, seconds since 2026-01-01 UTC)`, so a restored database never reuses an InvId already sent to Robokassa (allowed range up to 2^63−1).
- Config: DOMAIN must be a bare host name and PUBLIC_BASE_URL must not carry a port (MAX delivers only to 443, no port in the URL); ROBOKASSA_TEST=1 with a shop configured is a startup warning, and /healthz has an extra field `payments_test_mode`.
- Deletion and moderation (§11 'deletion', legal pages): `/forget USERID` (with confirmation) cancels the person's running games, takes them out of games they play in (splice), deletes their games, participations, pairs, exclusions, relays, reports, queued messages and profile, and strips them from payments and events. `/clean CODE USERID` and the report's [Стереть тексты отправителя] clear a person's wishes, display name (→ 'Участник') and relays in one game; /game has [Сбросить название]. Blocked users still edit their own wishes and name (as noted above).
- Privacy: the consent and privacy texts now say that the invite and the group card show the first names and the group reveal shows the pairs to the chat the organizer chose (CONSENT_VERSION left at 2026-10, as nothing has launched). invite_open is recorded without the user id before consent. Open complaints lose the quoted text after 30 days.
- Dependencies: requirements.txt also pins tzdata (Windows has no time zone database; the README demo crashed there) and the tested transitive dependencies of aiohttp and jinja2.
- Copy: 'Мои игры' numbers lines and buttons (default titles repeat); the draw confirmation agrees in number; 'Вы — Тайный Санта для: Мария К..' lost its double dot; 'можно спросить анонимно' is shown only while anonymous messages are on; the relay prompt keeps the name in the nominative ('Напишите сообщение своему получателю (Имя)'); the certificate alert names the file to replace.
